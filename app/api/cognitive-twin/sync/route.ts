 ...
 * Used by CognitiveTwinProvider on mount to hydrate the client state.
 */
export async function GET(_req: NextRequest) {
  try {
    ...
    return NextResponse.json({ twin: record })
  } catch (error) {
 ...
 * Persists an initial CognitiveTwin from onboarding to the DB.
 * Also accepts partial updates from client (e.g. workspaceLayout preference changes).
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { identity, energyCurve, metrics, bottlenecks, workspaceLayout, confidenceScore, isInitialized } = body
