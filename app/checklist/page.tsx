...
export default function ChecklistPage() {
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    if (newItemText.trim()) {
      ...
      const currentManual = JSON.parse(localStorage.getItem("novo_checklist_items") || "[]")
      localStorage.setItem("novo_checklist_items", JSON.stringify([...currentManual, newItem]))

      setNewItemText("")
      loadTasks() // Reload to get the new item with correct integrated structure
  ...
  const handleDelete = (id: string) => {
    ...
    if (task && task.source === "manual") {
      const currentManual = JSON.parse(localStorage.getItem("novo_checklist_items") || "[]")
      const updated = currentManual.filter((i: any) => i.id !== id)
      localStorage.setItem("novo_checklist_items", JSON.stringify(updated))
      loadTasks()
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              {sortedItems.map((item) => (
                <div
                  key={item.id}
                  className={`flex flex-col sm:flex-row sm:items-center gap-3 rounded-lg border p-3 md:p-4 transition-colors hover:bg-accent ${
                    item.source !== "manual" ? "bg-secondary/20" : ""
                  }`}
                >
