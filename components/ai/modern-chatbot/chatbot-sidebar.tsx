...
import { Bot, ChevronRight, Brain } from 'lucide-react'
...
export function ChatbotSidebar() {
    ...
    const stateColors = {
        ...
        ANALYZING: 'bg-primary/80 shadow-[0_0_15px_var(--primary-glow)] animate-pulse',
        PROPOSING: 'bg-primary/90 shadow-[0_0_15px_var(--primary-glow)]',
        EXECUTING: 'bg-primary shadow-[0_0_15px_var(--primary-glow)] animate-pulse',
        ...
        <div className="fixed right-0 top-0 bottom-0 z-50 flex pointer-events-none">
            ...
            <div
                className={`pointer-events-auto w-[400px] bg-[#0B0B0F]/90 backdrop-blur-2xl border-l border-white/10 flex flex-col transition-transform duration-300 ease-out bg-noise ${sidebarCollapsed ? 'translate-x-full' : 'translate-x-0'
                    } ${cognitiveState !== 'IDLE' ? 'shadow-glow-lg border-primary/30' : 'shadow-2xl'
                    }`}
            >
                {/* Header */}
                <div className="h-14 border-b border-white/5 flex items-center justify-between px-4 bg-[#0B0B0F]/50 backdrop-blur-md">
                    <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-lg bg-primary/10 flex items-center justify-center border border-primary/20 shadow-[0_0_10px_rgba(var(--primary),0.1)]">
                ...
                {cognitiveState !== 'IDLE' && (
                    <div className="absolute top-14 left-0 right-0 h-0.5 bg-primary/20 overflow-hidden z-10">
                        <div className="h-full bg-primary animate-progress-indeterminate shadow-[0_0_10px_rgba(var(--primary),0.8)]" />
                    </div>
                )}

                {/* Messages Area — overflow-x-hidden prevents horizontal blowout */}
                <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar p-4">
                    <div className="space-y-6 pb-4 max-w-full overflow-hidden">
                        {messages.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-[400px] text-center space-y-6 opacity-0 animate-in fade-in zoom-in-95 duration-700 fill-mode-forwards delay-100">
                                <div className="relative group">
                                    <div className="absolute inset-0 bg-primary/10 blur-3xl rounded-full animate-pulse-slow group-hover:bg-primary/20 transition-all duration-1000" />
                                    <div className="h-16 w-16 rounded-full bg-gradient-to-br from-primary/5 to-primary/5 border border-white/5 flex items-center justify-center backdrop-blur-md relative z-10 shadow-2xl group-hover:scale-105 transition-transform duration-500">
                                        <Brain className="h-8 w-8 text-primary/50" />
                                    </div>
                                </div>
                                <div className="space-y-2 max-w-[240px]">
                                    <p className="text-xs font-mono text-primary/80 tracking-widest uppercase">System Ready</p>
