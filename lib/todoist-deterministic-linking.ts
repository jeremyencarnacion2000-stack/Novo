...
export type LinkPorts = {
  getConnection: (userId: string, connectionId: string) => Promise<{ id: string; userId: string; provider: string; providerAccountId?: string | null; status?: string } | null>
...
export async function linkTodoistTask(input: { userId: string; connectionId: string; taskId: string; internalType: string; internalId: string }, ports: LinkPorts) {
  ...
  if (!c || c.userId !== input.userId || c.provider !== 'todoist' || c.status === 'disconnected' || !c.providerAccountId) throw new Error('Verified Todoist connection required')
